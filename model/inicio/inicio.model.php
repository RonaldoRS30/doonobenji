...
class InicioModel
{
    ...
    public function ObtenerDatosImpPCDelivery($data)
    {
        try
        {
            $query = "select `p`.`id_pedido` AS `id_pedido`,`p`.`id_tipo_pedido` AS `id_tipo_p`,`p`.`id_usu` AS `id_usu`,`p`.`fecha_pedido` AS `fecha_p`,`p`.`estado` AS `estado`,`pd`.`nomb_cliente` AS `nomb_c`,`pd`.`direccion`,`pd`.`telefono`,`pd`.`comentario` AS `comentario` from `tm_pedido` `p` join `tm_pedido_delivery` `pd` on `p`.`id_pedido` = `pd`.`id_pedido` where `p`.`id_pedido` = ? and `p`.`estado` = 'a' order by `p`.`id_pedido` desc";
