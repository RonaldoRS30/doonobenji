...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `usp_cajaAperturar` (IN `_flag` INT(11), IN `_idTie` INT(11), IN `_idUsu` INT(11), IN `_idCaja` INT(11), IN `_idTurno` INT(11), IN `_fechaA` DATETIME, IN `_montoA` DECIMAL(10,2))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `usp_comprasRegProveedor` (IN `_flag` INT(11), IN `_ruc` VARCHAR(13), IN `_razS` VARCHAR(100), IN `_direc` VARCHAR(100), IN `_telf` INT(9), IN `_email` VARCHAR(45), IN `_contc` VARCHAR(45), IN `_idProv` INT(11))  BEGIN
		DECLARE _duplicado INT DEFAULT 1;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `usp_configAreasProd` (IN `_flag` INT(11), IN `_idImp` INT(11), IN `_nombre` VARCHAR(45), IN `_estado` VARCHAR(5), IN `_idArea` INT(11))  BEGIN
	DECLARE _cont INT DEFAULT 0;
	DECLARE _cod0 INT DEFAULT 0;
	DECLARE	_cod1 INT DEFAULT 1;
	DECLARE	_cod2 INT DEFAULT 2;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `usp_configImpresoras` (IN `_flag` INT(11), IN `_nombre` VARCHAR(50), IN `_estado` VARCHAR(5), IN `_idImp` INT(11))  BEGIN
	DECLARE _cont INT DEFAULT 0;
	DECLARE _cod0 INT DEFAULT 0;
	DECLARE	_cod1 INT DEFAULT 1;
	DECLARE	_cod2 INT DEFAULT 2;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `usp_configProductoIngrs` (IN `_flag` INT(11), IN `_idPres` INT(11), IN `_idTIns` INT(11), IN `_idIns` INT(11), IN `_idMed` INT(11), IN `_cant` FLOAT, IN `_idPi` INT(11))  BEGIN
	if _flag = 1 then
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `usp_configProductoPres` (IN `_flag` INT(11), IN `_idProd` INT(11), IN `_codP` VARCHAR(45), IN `_presP` VARCHAR(45), IN `_precio` DECIMAL(10,2), IN `_rec` INT(1), IN `_stock` INT(11), IN `_estado` VARCHAR(5), IN `_img` VARCHAR(200), IN `_idPres` INT(1))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `usp_configTiendas` (IN `_flag` INT(11), IN `_nomb` VARCHAR(100), IN `_direc` VARCHAR(200), IN `_telef` VARCHAR(20), IN `_est` VARCHAR(5), IN `_idTie` INT(11))  BEGIN
	DECLARE _cont INT DEFAULT 0;
	DECLARE _cod0 INT DEFAULT 0;
	DECLARE	_cod1 INT DEFAULT 1;
	DECLARE	_cod2 INT DEFAULT 2;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `usp_configUsuario` (IN `_flag` INT(11), IN `_idTie` INT(11), IN `_idRol` INT(11), IN `_idArea` INT(11), IN `_dni` VARCHAR(10), IN `_apeP` VARCHAR(45), IN `_apeM` VARCHAR(45), IN `_nomb` VARCHAR(45), IN `_email` VARCHAR(100), IN `_usu` VARCHAR(45), IN `_cont` VARCHAR(45), IN `_img` VARCHAR(45), IN `_idUsu` INT(11))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `usp_invESAnular` (IN `_flag` INT(11), IN `_idEs` INT(11), IN `_idTo` INT(11))  BEGIN
	DECLARE _cont INT DEFAULT 0;
	IF _flag = 1 THEN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `usp_restEmitirVenta` (`_flag` INT(11), `_idDP` INT(11), `_idTE` INT(11), `_idPed` INT(11), `_idCli` INT(11), `_idTp` INT(11), `_idTd` INT(11), `_idUsu` INT(11), `_idApc` INT(11), `_pagoTar` DECIMAL(10,2), `_dscto` DECIMAL(10,2), `_bolsa` DECIMAL(10,2), `_igv` DECIMAL(10,2), `_total` DECIMAL(10,2), `_fecha` DATETIME)  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `usp_restRegCliente` (IN `_flag` INT(11), IN `_dni` VARCHAR(10), IN `_ruc` VARCHAR(13), IN `_apeP` VARCHAR(45), IN `_apeM` VARCHAR(45), IN `_nomb` VARCHAR(100), IN `_razS` VARCHAR(100), IN `_telf` INT(11), IN `_fecN` DATE, IN `_correo` VARCHAR(100), IN `_direc` VARCHAR(100), IN `_idCliente` INT(11))  BEGIN
		DECLARE _duplicado INT DEFAULT 1;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `usp_restRegDelivery` (IN `_flag` INT(11), IN `_idTp` INT(11), IN `_idUsu` INT(11), IN `_fechaP` DATETIME, IN `_nombC` VARCHAR(100), IN `_dirC` VARCHAR(100), IN `_motorC` VARCHAR(100), IN `_telfC` VARCHAR(20), IN `_comen` VARCHAR(100))  BEGIN
	IF _flag = 1 THEN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `vsp_clientes` ()  BEGIN
	SELECT
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_inventario`  AS  (select `e`.`id_tipo_ins` AS `id_tipo_ins`,`e`.`id_ins` AS `id_ins`,ifnull(`e`.`total`,0) AS `ent`,ifnull(`s`.`total`,0) AS `sal` from (`v_inventario_ent` `e` left join `v_inventario_sal` `s` on(`e`.`id_ins` = `s`.`id_ins`))) ;
