...
if(file_exists($archivo)){
    ...
    if($data->id_tdoc == 1){
        ...
        $pdf->CellFitScale(15, 3,''.number_format((0),2), 0, 1, 'R');
        $a = 7;
    }else{
        $pdf->SetXY(2, $h+$y+6+$z);//modificar solo esto
        $pdf->CellFitScale(55, 3,'Dscto: '.$_SESSION["moneda"], 0, 1, 'R');
    ...
    $pdf->CellFitScale(15, 3,number_format(($data->total - $data->descu + 0),2), 0, 1, 'R');
    $pdf->SetXY(2, $h+$y+6+$z+$a+6);//modificar solo esto
    $pdf->CellFitScale(70, 3,'----------------------------------------------', 0, 1, 'L');
    $pdf->SetXY(2, $h+$y+6+$z+$a+9);//modificar solo esto
    $pdf->CellFitScale(70, 3,'SON: '.numtoletras($data->total - $data->descu + 0), 0, 1, 'L');
